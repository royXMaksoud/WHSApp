...
namespace WHS.Infrastructure.Repositories.Code
{
    class CodeTableRepository(WarehouseDbContext dbContext) : ICodeTableRepository
    {
        ...
        {
            dbContext.CodeTables.Add(entity);
            await dbContext.SaveChangesAsync();
            return entity.TableGUID;
        }
        ...
        {
            var result = await dbContext.CodeTables.ToListAsync();
            return result;  
        }
        ...
        {
            var baseQuery = dbContext.CodeTables.Where(x => searchPhrase == null || x.TableName.ToLower().Contains(x.TableName));
        ...
        {
            var result = await dbContext.CodeTables.Include(x => x.CodeTableValues).FirstOrDefaultAsync(x => x.TableGUID == id);
            return result;
        }
